...
describe("MONDividendDistributor", function () {
  ...
  beforeEach(async function () {
    ...
    dividendDistributor = await MONDividendDistributor.deploy(tokenName, tokenSymbol);
  ...
  describe("MON Distribution", function () {
    beforeEach(async function () {
    ...
    it("Should emit DividendsDistributed event", async function () {
      ...
      await expect(
        ...
        .withArgs(user1.address, ethers.utils.parseEther("2")); // 20% of 10 MON
    });
  });
  ...
  describe("Dividend Withdrawal", function () {
    beforeEach(async function () {
      ...
      // Send MON to trigger distribution
      await user1.sendTransaction({
        to: dividendDistributor.address,
    ...
    it("Should allow users to withdraw their dividends", async function () {
      ...
      // Should receive 33.33% of 2 MON = ~0.667 MON
  ...
  describe("Proportional Distribution", function () {
    it("Should distribute dividends proportionally based on token holdings", async function () {
      ...
      // Send 10 MON (2 MON for dividends)
      await user1.sendTransaction({
        to: dividendDistributor.address,
        value: ethers.utils.parseEther("10")
      });
      ...
      // User1 should get 25% of 2 MON = 0.5 MON
      // User2 should get 75% of 2 MON = 1.5 MON
  ...
  describe("Edge Cases", function () {
    ...
    it("Should reject zero value transactions", async function () {
      ...
      ).to.be.revertedWith("No MON sent");
    });
  });
});
